...
import { generateContextualText, improveTextInContext, proofreadText, ProofreadSuggestion, queryDocument } from '../services/geminiService';
import { exportToTxt, exportToDocx } from '../services/documentExporter';
import { htmlToPlainText, formatNewCardContent } from '../services/documentParser';
...
type ModalState = AddEditModalState | { type: 'paste_document' } | { type: 'move_cards', cards: Card[], sourceProjectId: string } | { type: 'blueprint_library' } | { type: 'save_blueprint', project: Project } | null;
...
type WindowId = 'projects' | 'selections' | 'workspace';
...
const ProjectLibrary: React.FC<ProjectLibraryProps> = ({
    ...
    const anyModalOpen = useMemo(() => {
        return !!modalState || !!focusModalState || isImporting || !!compositionPanelCard || quickCaptureState.isOpen || isFocusPreviewOpen || isCarouselFocusOpen;
    }, [modalState, focusModalState, isImporting, compositionPanelCard, quickCaptureState.isOpen, isFocusPreviewOpen, isCarouselFocusOpen]);
    ...
    const visibleProjectOrder = useMemo(() => {
        ...
        const traverse = (parentId: string | null) => {
            ...
            for (const project of children) {
                // FIX: Changed 'p.isHidden' to 'project.isHidden' to correctly reference the loop variable.
        ...
        if (inboxProject) {
            ...
            if (expandedProjects.has(inboxProject.id)) {
                traverse(inboxProject.id);
            }
        }
        traverse(null);
        return order;
    ...
    useEffect(() => {
        if (isDesktop) {
            const updatePositions = () => {
                ...
                setPopoverPositions({
                    actions: { top: actionsRect?.top ?? 0 },
                    theme: { top: themeRect?.top ?? 0 }
                });
            };
    ...
    useEffect(() => {
        const handleWindowTouchMove = (e: TouchEvent) => {
            ...
            if (currentlyOverProjectId) {
                ...
                const cardWrappers = Array.from(container?.querySelectorAll('[data-card-id]') || []);
        ...
        const handleWindowTouchEnd = () => {
            ...
            if (touchDragState.draggedOverProjectId && touchDragState.draggedCard) {
                ...
                const cardWrappers = Array.from(touchDragState.placeholderElement.parentElement.children).filter(el => el.hasAttribute('data-card-id') || el === touchDragState.placeholderElement);
                const orderedIds = cardWrappers.map(el => el === touchDragState.placeholderElement ? touchDragState.draggedCard!.id : el.getAttribute('data-card-id'));
    ...
    const allCardsForActiveHierarchy = useMemo(() => {
        ...
        const cardsWithContext: HoveredCardInfo[] = [];
    ...
    const createMarkup = (htmlContent: string) => {
        const content = (htmlContent || '').trim() === '' ? '&nbsp;' : htmlContent;
    ...
    const previewHtmlWithCardIds = useMemo(() => {
        const htmlParts = hierarchicalCards.map(card => {
            ...
            const sanitizedHtml = DOMPurify.sanitize(content);
            return `<div class="card-source-wrapper" data-card-id="${card.id}">${sanitizedHtml}</div>`;
        });
    ...
    const handleOpenCardFlow = (cardId: string) => {
        const cardIndex = visibleCards.findIndex(c => c.id === cardId);
        if (cardIndex !== -1) {
            ...
            <>
                ...
                <div className="flex flex-col min-h-0 flex-grow mt-4">
                     <div className="flex justify-between items-center mb-4 flex-shrink-0">
                        <div className="flex items-center space-x-2">
                            <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Saved Selections</h3>
                            <div className="flex items-center space-x-1 bg-[var(--color-element-primary)] p-0.5 rounded-lg">
                                ...
                                <button onClick={() => { setInitialCarouselIndex(0); setIsCarouselFocusOpen(true); }} title="Card Flow" className="p-1.5 rounded-md transition-colors text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]/70"><LayoutTemplateIcon className="w-5 h-5" /></button>
                            </div>
                        </div>
                        <div className="flex items-center space-x-2">
                            {hiddenCardCount > 0 && (
                    ...
                    <div ref={selectionsContainerRef} onDragOver={handleDragOverSelections} onDragLeave={stopAutoScroll} className="flex-grow overflow-y-auto pr-2 space-y-2">
                         {!searchQuery && visibleCards.length > 0 && <div className="h-4 my-[-6px] relative flex items-center justify-center cursor-pointer group" role="button" onClick={() => setFocusModalState({ mode: 'add', insertIndex: 0 })}><div className="w-full h-0.5 bg-transparent group-hover:bg-[var(--color-accent-primary)] opacity-0 group-hover:opacity-100 transition-all duration-200" /><div className="absolute flex items-center justify-center h-6 w-6 bg-[var(--color-bg-secondary)] border-2 border-transparent rounded-full opacity-0 group-hover:opacity-100 group-hover:border-[var(--color-accent-primary)] transition-all duration-200 scale-50 group-hover:scale-100"><PlusIcon className="w-4 h-4 text-[var(--color-accent-primary)]" /></div></div>}
                        {visibleCards.map((card, index) => {
                            ...
                            return (
                                <React.Fragment key={card.id}>
                                    ...
                                    <div className="relative group/card py-1" data-card-id={card.id}>
                                        <div ref={(el) => { if (el) cardRefs.current.set(card.id, el); else cardRefs.current.delete(card.id); }} draggable={!card.isLocked && selectedCardIds.size <= 1} onDragStart={(e) => handleDragStartCard(e, card)} onDragEnd={handleDragEndCard} onDragOver={(e) => e.preventDefault()} onDrop={(e) => !card.isLocked && !searchQuery && handleDropOnCardReorder(e, card.id)} onDoubleClick={(e) => handleCardDoubleClick(e, card)} onTouchStart={(e) => handleTouchStartCard(e, card)} onTouchMove={handleTouchMoveCard} onTouchEnd={handleTouchEndOrCancelCard} onTouchCancel={handleTouchEndOrCancelCard} className={`flex items-start space-x-3 p-4 border rounded-lg transition-all duration-200 shadow-sm hover:shadow-xl hover:-translate-y-1 ${card.isLocked ? 'cursor-not-allowed' : 'cursor-grab'} ${card.color ? `${CARD_COLORS[card.color]?.bg || 'bg-[var(--color-bg-secondary)]'} ${CARD_COLORS[card.color]?.border || 'border-[var(--color-border-primary)]'}` : 'bg-[var(--color-bg-secondary)] border-[var(--color-border-primary)]'} ${selectedCardIds.has(card.id) ? '!border-[var(--color-accent-primary)] ring-2 ring-[var(--color-ring)]' : ''} ${draggedCardInfo?.cards.some(c=>c.id === card.id) ? 'opacity-30 scale-95' : 'opacity-100'} ${card.isHidden ? 'opacity-50' : ''}`} title={card.isLocked ? "This card is locked. Unlock it from the options menu to edit." : ""}>
                                            <input type="checkbox" checked={selectedCardIds.has(card.id)} onChange={() => toggleCardSelection(card.id)} disabled={card.isLocked} className="mt-1.5 flex-shrink-0 form-checkbox h-4 w-4 bg-[var(--color-bg-tertiary)] border-[var(--color-border-secondary)] text-[var(--color-accent-primary)] focus:ring-[var(--color-accent-primary)] rounded disabled:opacity-50" />
                                            <div className="flex-grow">
                                                    ...
                                                    <div className={`flex-grow pt-0.5 text-[var(--color-text-primary)] text-base`}>
                                                        {searchQuery && !searchQuery.startsWith('#') ? (
                                                            ...
                                                            <div
                                                                ...
                                                                dangerouslySetInnerHTML={createMarkup(card.content)}
                                                            />
                                                        )}
                                                    </div>
                                                    <div className="flex-shrink-0">
                                                        {card.isHidden ? (
                                                            ...
                                                            <div className="grid grid-cols-2 gap-x-1 gap-y-0.5">
                                                                <button onClick={() => handleShareCard(card)} title="Share Card" className="p-1.5 rounded-full hover:bg-[var(--color-accent-subtle-bg)] text-[var(--color-accent-primary)] hover:text-[var(--color-accent-primary-hover)]"><ShareIcon className="w-4 h-4" /></button>
                                                                        ...
                                                                        <div className="absolute top-full right-0 mt-2 w-56 bg-[var(--color-bg-secondary-alpha)] backdrop-blur-sm border border-[var(--color-border-primary)] rounded-lg shadow-xl z-20 p-2">
                                                                            <div className="grid grid-cols-4 gap-2 mb-2">
                                                                                {Object.keys(CARD_COLORS).map(color => (
                                                                                    <button key={color} onClick={() => { onUpdateCard(card.projectId, card.id, { color }); setActiveCardOptions(null); }} className={`w-full h-8 rounded ${CARD_COLORS[color].bg} border-2 ${card.color === color ? CARD_COLORS[color].border : 'border-transparent'} hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed`} disabled={card.isLocked}></button>
                                                                                ))}
                                                                ...
                                                                <button onClick={() => handleOpenCardFlow(card.id)} title="Open in Card Flow" className="p-1.5 rounded-full hover:bg-[var(--color-accent-subtle-bg)] text-[var(--color-accent-primary)] hover:text-[var(--color-accent-primary-hover)] disabled:opacity-50" disabled={card.isLocked}>
                                                ...
                                                {card.tags && card.tags.length > 0 && (
                                                    <div className="mt-3 flex flex-wrap gap-2">
                                                        {card.tags.map(tag => (
                                                            <button key={tag} onClick={() => handleTagClick(tag)} className="text-xs bg-[var(--color-accent-subtle-bg)] text-[var(--color-accent-subtle-text)] font-medium px-2.5 py-1 rounded-full hover:opacity-80 transition-opacity">
                                                                #{tag}
                                                            </button>
                                                        ))}
                                                    </div>
                                        ...
                                        {!searchQuery && !card.isHidden && !card.isLocked && <button onClick={() => setFocusModalState({ mode: 'add', insertIndex: (activeProject?.cards.findIndex(c => c.id === card.id) ?? -1) + 1 })} className="absolute -bottom-2.5 left-1/2 -translate-x-1/2 h-5 w-5 bg-[var(--color-bg-secondary)] border-2 border-[var(--color-border-secondary)] rounded-full flex items-center justify-center opacity-0 group-hover/card:opacity-100 hover:!opacity-100 hover:border-[var(--color-accent-primary)] hover:scale-110 transition-all duration-150 z-10"><PlusIcon className="w-3 h-3 text-[var(--color-text-secondary)] hover:text-[var(--color-accent-primary)]" /></button>}
                                    </div>
                                </React.Fragment>
                            )
                        })}
    ...
    return (
        <main className={`h-screen w-screen font-sans ${theme}`}>
                    ...
                    <>
                        ...
                        {isActionsPopoverOpen && (
                            <div
                                ...
                                className="absolute left-24 z-50 bg-[var(--color-bg-secondary-alpha)] backdrop-blur-lg border border-[var(--color-border-primary)] rounded-lg shadow-xl w-60 text-[var(--color-text-primary)] animate-fade-in"
                        ...
                        {isThemePopoverOpen && (
                            <div
                                ref={themePopoverRef}
                                className="absolute left-24 z-50 bg-[var(--color-bg-secondary-alpha)] backdrop-blur-lg border border-[var(--color-border-primary)] rounded-lg shadow-xl w-48 text-[var(--color-text-primary)] animate-fade-in"
            ...
            {modalState && modalState.type !== 'save_blueprint' && modalState.type !== 'blueprint_library' && modalState.type !== 'move_cards' && modalState.type !== 'paste_document' && ( <ProjectModal modalState={modalState} onClose={handleCloseModalState} onAddProject={onAddProject} onUpdateProject={onUpdateProject} projects={projects} /> )}
            ...
            {isCarouselFocusOpen && ( <CarouselView cards={visibleCards} initialIndex={initialCarouselIndex} onClose={handleCloseCarouselView} onUpdateCard={onUpdateCard} onDeleteCard={handleCardDelete} onDuplicateCard={onDuplicateCard} onShareCard={handleShareCard} onCompositionEdit={(card) => { handleCloseCarouselView(); setFocusModalState({ mode: 'edit', card, projectId: card.projectId }); }} onNavigateBackToCard={handleNavigateBackToCard} /> )}
...
// FIX: Add a default export for the component.
